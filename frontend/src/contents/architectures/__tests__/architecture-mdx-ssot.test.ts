...
import { publishedArchitectures } from "../architectures";

/**
 * architecture MDX (contents/architectures/*.mdx) と publishedArchitectures (TS)
 * の slug 集合が完全一致することを検証する invariant test。
 *
 * 背景: isExistingArchitecture は publishedArchitectures を参照する一方、
 * architecture 詳細ルート (/architectures/[architectureSlug]) の notFound() は
 ...
 */
function getArchitectureMdxDirectory(): string {
  const candidates = [
    path.join(process.cwd(), "contents", "architectures"),
    path.join(process.cwd(), "frontend", "contents", "architectures"),
  ];
  ...
  throw new Error("architecture MDX directory not found");
}
...
function getMdxSlugs(): string[] {
  const directory = getArchitectureMdxDirectory();
...
describe("architecture MDX ↔ publishedArchitectures SSoT invariant", () => {
  const mdxSlugs = getMdxSlugs();
  const tsSlugs = publishedArchitectures.map((a) => a.slug).sort();

  it("MDX slug set equals publishedArchitectures slug set", () => {
  ...
  it("publishedArchitectures slug set has no duplicates", () => {
