...
CloudFrontは配信とキャッシュの前段です。アプリケーションの処理を実行するALB、HTTP APIを公開するAPI Gateway、ファイルを保存するS3とは担当が異なります。画像、JavaScript、HTMLの配信はCloudFrontとS3の組み合わせが分かりやすく、ログイン後の動的画面はCloudFrontの背後にALBを置く構成もあります。Lambdaを呼ぶ問い合わせAPIは、API Gatewayを入口にするのが自然です。
